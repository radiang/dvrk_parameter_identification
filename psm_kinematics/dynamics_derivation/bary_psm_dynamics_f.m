...
dyn.r_cg = sym('r_cg%d_%d', [dyn.num 3], 'real');
dyn.r_cg = [dyn.r_cg,ones(dyn.num,1)];
%Inertia Matrix notation
dyn.Lxx = sym('L%d_xx', [dyn.num 1],'real');
dyn.Lyy = sym('L%d_yy', [dyn.num 1],'real');
dyn.Lzz = sym('L%d_zz', [dyn.num 1],'real');
dyn.Lxy = sym('L%d_xy', [dyn.num 1],'real');
dyn.Lxz = sym('L%d_xz', [dyn.num 1],'real');
dyn.Lyz = sym('L%d_yz', [dyn.num 1],'real');

for i=1:1:length(dyn.Lxx)
    
    dyn.L(1:3,1:3,i)=[dyn.Lxx(i),dyn.Lxy(i),dyn.Lxz(i); 
              dyn.Lxy(i),dyn.Lyy(i),dyn.Lyz(i);
              dyn.Lxz(i),dyn.Lyz(i),dyn.Lzz(i);]; 
end

for i = 1:length(dyn.Lxx)
    S = [0, -dyn.r_cg(i,3), dyn.r_cg(i,2)   ;
       dyn.r_cg(i,3)  ,0 , -dyn.r_cg(i,1)   ;
       -dyn.r_cg(i,2)  , dyn.r_cg(i,1)    , 0  ]; 
   
    dyn.I(:,:,i) = dyn.L(:,:,i) - 1./dyn.M(i)*S.'*S;
    dyn.l_cg(i,1:3) = dyn.r_cg(i,1:3)./dyn.M(i);
end
...
dyn.Dt=collect(dyn.Dt,[dyn.Lxx,dyn.Lxy,dyn.Lxz,dyn.Lyy,dyn.Lyz,dyn.Lzz,reshape(dyn.r_cg,[1,numel(dyn.r_cg)]),dyn.M,dyn.Fv,reshape(dyn.Fvl,1,[]),dyn.Fs,dyn.Ke]);
