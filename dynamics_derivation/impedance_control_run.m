...
D_t = subs(dyn.D, [gen.q, gen.qd, gen.qdd], [qt, qdt, qddt]);
C_t = subs(dyn.C, [gen.q, gen.qd, gen.qdd], [qt, qdt, qddt]);
Psi_t = subs(dyn.Psi, [gen.q, gen.qd, gen.qdd], [qt, qdt, qddt]);
...
l_cg_num = 0.02*ones(1,numel(dyn.l_cg(:,1:3)));
...
psm.l_cg(2,:) = [   -0.012175*cos(dyn.beta)-0.036897*sin(dyn.beta)  ,  +0.012175*sin(dyn.beta)-0.036897*cos(dyn.beta)    ,0]; %
psm.l_cg(3,:) = [   0.010348*cos(dyn.beta)+0.256832*sin(dyn.beta)  ,-0.010348*sin(dyn.beta)+0.256832*cos(dyn.beta), 0 ]; %
...
%% Substitution
D_t = subs(D_t, [ reshape(dyn.l_cg(:,1:3),1,[]),transpose(dyn.Ixx), transpose(dyn.Ixy), transpose(dyn.Ixz), transpose(dyn.Iyy), transpose(dyn.Iyz), transpose(dyn.Izz), dyn.M ] ,[reshape(psm.l_cg(:,1:3),1,[]),psm.Ixx,psm.Ixy,psm.Ixz,psm.Iyy,psm.Iyz,psm.Izz, psm.m]);
C_t = subs(C_t, [ reshape(dyn.l_cg(:,1:3),1,[]),transpose(dyn.Ixx), transpose(dyn.Ixy), transpose(dyn.Ixz), transpose(dyn.Iyy), transpose(dyn.Iyz), transpose(dyn.Izz), dyn.M ] ,[reshape(psm.l_cg(:,1:3),1,[]),psm.Ixx,psm.Ixy,psm.Ixz,psm.Iyy,psm.Iyz,psm.Izz, psm.m]);
Psi_t = subs(Psi_t, [ reshape(dyn.l_cg(:,1:3),1,[]),transpose(dyn.Ixx), transpose(dyn.Ixy), transpose(dyn.Ixz), transpose(dyn.Iyy), transpose(dyn.Iyz), transpose(dyn.Izz), dyn.M ] ,[reshape(psm.l_cg(:,1:3),1,[]),psm.Ixx,psm.Ixy,psm.Ixz,psm.Iyy,psm.Iyz,psm.Izz, psm.m]);

%eqn = D_t(1:3,1:3) * qddt(1:3)' + C_t(1:3,1:3)*qdt(1:3)'+ Psi_t(1:3);
eqn = D_t(1:3,1:3) * transpose(qddt(1:3));

ctrl.xe = subs(dyn.T(1:3,4,11), gen.q, qt);
...
init.x0n = double(subs(dyn.T(1:3,4,11), gen.q, q0.'));
...
ctrl.u = impedence_control(ctrl,joint,init);
...
[time, ctrl.q_out] = ode45(Ffun, [0 7], [q0(1:3);qd0(1:3)], opt);


%% Plot Output
beg = 1;
ends = length(ctrl.q_out);

ctrl.T_out = subs(dyn.T(1:3,4,11),[gen.q(4) gen.q(5) gen.q(6)],[0 0 0]);
