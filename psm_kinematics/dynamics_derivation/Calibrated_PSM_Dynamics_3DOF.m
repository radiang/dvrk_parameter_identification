...
%Jacobian Notation
for i=1:dof+2
    %Jv(1:3,1:dof,i)=zeros(3,dof);
    
    Jv(1:3,1:dof,i)=[diff(X(:,i),q1),diff(X(:,i),q2),diff(X(:,i),q3),diff(X(:,i),q4),diff(X(:,i),q5),diff(X(:,i),q6)];
...
%%Potential Energy
P=m1*lc1*cos(thet1)*cos(q1)+m2*lc2*cos(q2)*cos(q1)+m3*l2*cos(q2)*cos(q1)+m4*lc4*cos(q2)*cos(q1)+m5*(lc5-q3)*cos(q2)*cos(q1)+m6*(lc6-q3)*cos(q2)*cos(q1)+m7*((-l5-q3)*cos(q2)*cos(q1)-lc7*cos(q5))+m8*((-l5-q3)*cos(q2)*cos(q1)-lc7*cos(q5)-lc8*cos(q6))+m9*((lc9+q3)*cos(q2)*cos(q1));
for i=1:1:6
Psi(i,1)=diff(P,Q(i));
end
...
Par=symvar(Dt);
Par(end-17:end)=[]; 
[Y, tau]=equationsToMatrix(Dt == Tau, Par);
