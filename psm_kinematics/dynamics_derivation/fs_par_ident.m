function [gen,fs,ident]=fs_par_ident(gen,fs,ident,plot_on)
...
%testname = 'PID_data2';
csvname = strcat(foldername,gen.fstestname,'.csv');
q=csvread(csvname);
...
%% Get Derivative (Now try to get accelerations from fourier fft spectrum)
...
for i=1:length(q(:,1))-10 
    W(1+(i-1)*dof_num:dof_num+(i-1)*dof_num,:)=gen.condfun(q(i,1),q(i,2),q(i,3),qdf(i,1),qdf(i,2),qdf(i,3),acc(i,1),acc(i,2),acc(i,3));
     W(1+(i-1)*dof_num:dof_num+(i-1)*dof_num,:) =  W(1+(i-1)*dof_num:dof_num+(i-1)*dof_num,:).*fs.scale.';
...
tau = reshape(q(:,7:9).',1,[]);
gen.par_cond=cond(W*P);
gen.par_num = pinv(W*P)*tau(1:length(W)).';

ident.acc = acc;
ident.qdf = qdf;
ident.q=q;

%% Will I try Semidefinite Programming Here? 
